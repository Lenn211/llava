...
class RobotInspectorGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Robot Inspector - Live Camera + Automated Inspection")
        ...
        self.camera_source = tk.StringVar(value="0")  # Default to webcam 0
        ...
        title_label = tk.Label(
            self.main_frame, 
            text="Robot Inspector - Live Camera",
            font=('Arial', 14, 'bold'),
            bg='#2c3e50',
            fg='#ecf0f1'
        )
        ...
        self.camera_label = tk.Label(camera_frame, bg='#2c3e50', text="Camera off")
        ...
        grayscale_s_radio = tk.Radiobutton(
            radio_frame,
            text="YOLOv11s-Gray",
            variable=self.selected_model,
            value="yolov11s_grayscale_from_scratch.pt",
            bg='#34495e',
            fg='#ecf0f1',
            selectcolor='#2c3e50',
            activebackground='#34495e',
        ...
        grayscale_s_radio.pack(side=tk.LEFT, padx=3)
    ...
    def start_camera(self):
        ...
        try:
            ...
            self.update_status("Opening camera...", '#f39c12')
            ...
            if not self.camera.isOpened():
                error_msg = f"Failed to open camera: {original_source}"
            ...
            self.update_status("Testing camera connection...", '#f39c12')
            ret, test_frame = self.camera.read()
            
            if not ret or test_frame is None:
                error_msg = f"Camera opened but cannot read frames: {original_source}"
                print(f"[ERROR] {error_msg}")
                self.update_status(error_msg, '#e74c3c')
                self.camera.release()
                self.camera = None
            ...
            self.update_status("Camera running", '#27ae60')
            ...
            self.update_status(f"Camera error: {str(e)}", '#e74c3c')
    ...
    def stop_camera(self):
        ...
        self.update_status("Camera stopped", '#95a5a6')
    ...
    def camera_loop(self):
        ...
        try:
            while self.camera_active:
                ...
                time.sleep(0.2)  # ~5 FPS to reduce load significantly
    ...
    def on_detection_select(self, event):
        ...
        print(f"[DEBUG] on_detection_select triggered")
        ...
        if idx >= len(self.current_detections):
            print("[DEBUG] Index out of range, returning")
            return
        
        detection = self.current_detections[idx]
        print(f"[DEBUG] Selected detection: {detection['class_name']} @ {detection['confidence']:.2f}")
        ...
        if self.current_frame is not None:
            ...
            print(f"[DEBUG] Cropped region: ({x1},{y1}) to ({x2},{y2}), shape: {cropped.shape}")
            ...
            print(f"[DEBUG] Cropped image displayed")
            
            # Enable inspect button
            self.inspect_button.config(state=tk.NORMAL, bg='#e67e22')
            print(f"[DEBUG] INSPECT button enabled")
        else:
            print("[DEBUG] No current_frame available!")
